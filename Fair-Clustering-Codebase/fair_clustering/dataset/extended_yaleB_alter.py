...
class ExtendedYaleB_alter(ImageDataset):
    """ http://vision.ucsd.edu/~iskwak/ExtYaleDatabase/ExtYaleB.html """

    dataset_name = "Extended Yale B"
    dataset_dir = os.path.join(sys.path[1], "fair_clustering/raw_data/extended_yaleB_alter")
    file_url = {
        "yaleB_alter.zip": "http://vision.ucsd.edu/extyaleb/CroppedYaleBZip/CroppedYale.zip",
    }

    def __init__(self, download=False, center=True, resize=False):
        self.download_or_check_data(self.dataset_dir, self.file_url, download)

        dataset_dir = os.path.join(self.dataset_dir, "yaleB_alter")
        ...
        super(ExtendedYaleB_alter, self).__init__(
            X=X,
            y=y,
            s=s,
            center=center,
