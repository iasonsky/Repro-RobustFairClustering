...
from fair_clustering.dataset import ExtendedYaleB, Office31, MNISTUSPS, MTFL, ExtendedYaleB_alter
...
def save_attack_data(name, cl_algo, pre_attack_res, post_attack_res, random_attack_res):
    ...
    folder_path = os.path.join('extra_metrics_results', name, cl_algo)
...
for trial_idx in range(n_trials):
  ...
  pa_bal, pa_min_cl_rat, pa_dist_l1, pa_dist_kl, pa_sil_diff, pa_ent, pa_ent_a, pa_ent_b, pa_acc, pa_nmi, pa_ari, pa_sil = process_solution(solution)
  ...
  r_bal, r_min_cl_rat, r_dist_l1, r_dist_kl, r_sil_diff, r_ent, r_ent_a, r_ent_b, r_acc, r_nmi, r_ari, r_sil = conduct_random_attack(dim_size)
  random_attack_res['BALANCE'].append(r_bal)
  random_attack_res['MIN_CLUSTER_RATIO'].append(r_min_cl_rat)
  random_attack_res['CLUSTER_DIST_L1'].append(r_dist_l1)
  random_attack_res['CLUSTER_DIST_KL'].append(r_dist_kl)
  random_attack_res['SILHOUETTE_DIFF'].append(r_sil_diff)
  random_attack_res['ENTROPY'].append(r_ent)
  random_attack_res['ENTROPY_GROUP_A'].append(r_ent_a)
  random_attack_res['ENTROPY_GROUP_B'].append(r_ent_b)
  random_attack_res['ACC'].append(r_acc)
  random_attack_res['NMI'].append(r_nmi)
  random_attack_res['ARI'].append(r_ari)
  random_attack_res['SIL'].append(r_sil)
...
print_results(random_attack_res, 'Random Attack', name, cl_algo)
